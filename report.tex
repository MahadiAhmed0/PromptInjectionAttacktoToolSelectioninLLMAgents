...
first be retrieved. The second part of the study therefore crafts
documents automatically, in two phases matching the two pipeline stages:

\begin{enumerate}
  \item \textbf{Retrieval aligned text.} An LLM is asked to summarize the
  core functionality implied by a small set of example queries for the
  target task. The resulting text is semantically aligned with those
  queries, which makes the document rank highly under embedding
  similarity.
  \item \textbf{Selection instruction.} A short instruction sequence is
  refined so that the selection LLM chooses the crafted tool once the
  document is in the candidate set. Two search strategies are supported:
  a \emph{gradient free} variant search, where an LLM proposes
  alternative phrasings and the selection LLM evaluates each candidate,
  keeping the best; and a \emph{gradient based} token optimization, where
  individual tokens of the instruction are updated against a local
  language model using a loss that balances three terms: likelihood of
  emitting the target tool name, likelihood of emitting the full expected
  JSON answer, and a readability penalty that keeps the text natural.
\end{enumerate}

The final document is the concatenation of the two parts. Figure
\ref{fig:crafting} sketches the process.
